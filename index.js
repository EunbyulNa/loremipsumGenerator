...
e.preventDefault();
  ...
  const value = parseInt(amount.value); //Change value string to number
  const random = Math.floor(Math.random() * text.length);

  if(isNaN(value) || value < 0 || value > 9){
    result.innerHTML = `<p class="reslut">${text[random]}</P>`
  }else{
    let finalTxt = text.slice(0, value); //slice(start,end)
    finalTxt = finalTxt.map( function(paragraphs) {
      return `<p class="reslut">${paragraphs}</P>`;
    }).join('');
